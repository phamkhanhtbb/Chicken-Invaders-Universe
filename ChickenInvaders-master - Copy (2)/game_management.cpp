...
void game_management::init(std::string title){
    ...
    playButton.loadImg("image/play.png", gRenderer);
    pauseButton.SetRect(SCREEN_WIDTH - 50, 10, 40, 40);
    playButton.SetRect(SCREEN_WIDTH - 50, 10, 40, 40);

    // Khởi tạo gift
    gift.loadImgGift(gRenderer);
    gift.set_y_val(GIFT_SPEED);
    gift.SetRect(rand()%(SCREEN_WIDTH-GIFT_WIDTH), -GIFT_HEIGHT);
...
void game_management::handle_event() {
    while(SDL_PollEvent(&gEvent)) {
        ...
        if(gEvent.type == SDL_MOUSEBUTTONDOWN) {
            ...
            SDL_Rect pauseRect = pauseButton.GetRect();
