...
void game_management::render_pause_overlay() {
    ...
    SDL_SetRenderDrawColor(gRenderer, 0, 0, 0, 128);  // Semi-transparent black
...
void game_management::handle_game() {
    ...
    // Xử lý màu nền và làm sạch renderer
    SDL_SetRenderDrawColor(gRenderer, RENDER_DRAW_COLOR, RENDER_DRAW_COLOR, RENDER_DRAW_COLOR, RENDER_DRAW_COLOR);
    SDL_RenderClear(gRenderer);

    // Xử lý background scrolling
    ...
    // Logic game chỉ chạy khi không pause
    if (!isPaused) {
        // Di chuyển và hiển thị spaceship
        spaceship.Move();
        ...
        if (spaceship.get_heart() == 0) {
            ...
            // Hint hồi sinh khi spaceship chết
        ...
        if (Col4) {
            ...
            if (bullet_level < 3 &&
                (gift.get_gift_type() == spaceship.get_bul_type() || gift.get_gift_type() == LEVEL_UP)) {
                bullet_level++;
            }
        ...
        // Render overlay pause nếu game đang pause
